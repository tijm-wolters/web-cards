...
impl TicTacToe {
  ...
  pub fn init(&mut self, game: &mut Game) {
    ...
    let message: types::JsonMessage = types::JsonMessage {
       ...
       data: types::Data::TicTacToeStarted(
        types::JsonTicTacToeStarted {
  ...
  pub fn handle(&mut self, game: &mut Game, msg: &types::IncomingMessage) {
    ...
    if r#move.x > 2 || r#move.y > 2
      ...
      utils::send_json_message(
        ...
        if self.nth_move % 2 == 0 { self.player_x.as_ref() } else { self.player_o.as_ref() }
      );
      return;
    };
    ...
    // Process the move
    self.nth_move += 1;
    self.board[r#move.x][r#move.y] = if self.nth_move % 2 == 0 { NodeState::X } else { NodeState::O };

    let move_message = types::JsonMessage {
      r#type: types::Type::TicTacToeMove,
      data: { types::Data::TicTacToeMoveSuccess(
        types::JsonTicTacToeMoveSuccess {
          x: r#move.x,
          y: r#move.y,
